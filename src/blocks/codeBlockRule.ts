...
function testStart(state: BlockParserState, parent: MarkdownNode) {
	...
	if (markup && markup.length >= 3) {
		...
		let codeNode = newBlockNode(name, state.i, state.line, 1, markup, state.indent);
		codeNode.subindent = state.indent;
		codeNode.acceptsContent = true;
		if (attributes.length) {
			codeNode.attributes = attributes;
		}
		delete state.attributes;

		let level = state.openNodes.indexOf(parent);
		if (state.blankLevel !== -1 && state.blankLevel <= level) {
			codeNode.blankBefore = true;
			state.blankLevel = -1;
		}

		parent.children!.push(codeNode);
		state.openNodes.push(codeNode);
